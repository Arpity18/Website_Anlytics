...
function MFWebFraudAsideMenuContent({
  ...
  const fetchMenuData = async () => {
    try {
      ...
      if (!token) {
       ...
       let productName = "Website Analytics";
  ...
  useEffect(() => {
    ...
    const skipMenuValidationRoutes = [
      ...
      "/webfraud/user-management/product-mapping",
    ];
    if (
    ...
    <div
      className={clsx(
        "z-[10] flex h-full flex-col bg-clip-border text-gray-300 shadow-xl transition-all",
        {
          ...
          "fixed bottom-0 left-0 top-14 w-[14rem] max-w-[14rem] p-2 md:relative md:top-auto":
            isExpanded,
    ...
    <div className="relative my- block w-full">
      ...
      >
        <button
          type="button"
          className={clsx(
            "relative flex w-full select-none items-center border-b-0 py-0.5 px-1 text-xs text-left font-sans text-base font-semibold leading-snug antialiased rounded-md group",
