...
const DeliveryOptionsModalContent = ({
  ...
  const { result: occurrenceApi, loading: occurrenceLoading } = useApiCall({
    url: type === "schedule" 
      ? process.env.NEXT_PUBLIC_APP_PERF + `reporting_tool/scheduler/occurance`
      : process.env.NEXT_PUBLIC_APP_PERF + `reporting_tool/download/occurance`,
    method: "POST",
    params: {
  ...
  const { result: mailingListApi, loading: mailingListLoading } = useApiCall({
    // url: `https://uat-api-dev.mfilterit.net/v1/app/reporting_tool/list_all_mailing_lists`,
    url: process.env.NEXT_PUBLIC_APP_PERF + `reporting_tool/list_all_mailing_lists`,
    method: "POST",
    manual: true,
    onSuccess: (data) => {
  ...
  }, [mailingListApi, selectedPackage, frequency, category]);
    ...
    if (defaultData) {
      ...
      if (email?.status) {
        ...
        if (mailinglist.length > 0) {
          ...
          const installLists = installMailingListNames.map((listName: string) => {
          ...
          const eventLists = eventMailingListNames.map((listName: string) => {
            const found = mailinglist.find((ml) => ml.mailing_list_name === listName);
            return found?.id;
          }).filter(Boolean) || [];
  ...
  useEffect(() => {
    if (defaultData?.email?.status && mailinglist.length > 0) {
      // Handle install mailing lists with fallback to generic mail_id_list
      const installMailingListNames = defaultData.email.install_mail_id_list || defaultData.email.mail_id_list || [];
      ...
      // Handle event mailing lists with fallback to generic mail_id_list
      const eventMailingListNames = defaultData.email.event_mail_id_list || defaultData.email.mail_id_list || [];
  ...
  const handleInstallMailingListToggle = useCallback((listId: string) => {
    if (mode === "view") return;
    ...
    setSelectedInstallMailingLists((prev: string[]) => {
      ...
      // Clear email error when mailing list is selected
  ...
  const handleEventMailingListToggle = useCallback((listId: string) => {
    if (mode === "view") return;
  ...
  const removeInstallMailingList = (listId: string) => {
    if (mode === "view") return;
    setSelectedInstallMailingLists((prev: string[]) => prev.filter((id: string) => id !== listId));
  };

  const removeEventMailingList = (listId: string) => {
    if (mode === "view") return;
    setSelectedEventMailingLists((prev: string[]) => prev.filter((id: string) => id !== listId));
  };
  ...
  const handleConfirm = () => {
    ...
    if(sendViaEmail) {
      ...
      if (categoryIncludes("install")) {
        ...
        // if (hasInstallEmails && !validateInstallEmails()) {
      ...
      if (categoryIncludes("event")) {
        ...
        // if (hasEventEmails && !validateEventEmails()) {
                        ...
                        <div className="absolute z-50 w-full mt-1 bg-popover border rounded-md shadow-md max-h-60 overflow-auto">
                          {mailingListLoading ? (
                            ...
                            mailinglist.map((list) => {
                              const isSelected = selectedInstallMailingLists.includes(list.id);
                      ...
                      {isEventDropdownOpen && (
                        <div className="absolute z-50 w-full mt-1 bg-popover border rounded-md shadow-md max-h-60 overflow-auto">
                          {mailingListLoading ? (
                            ...
                            mailinglist.map((list) => {
                              const isSelected = selectedEventMailingLists.includes(list.id);
                    ...
                    <div className="space-y-2">
                      <Label className="dark:text-white">Secret Key</Label>
                         ...
                         onChange={(e) => {
                           ...
                           // Clear cloud error when user types
